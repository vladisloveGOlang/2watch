...
import (
	...
	le "github.com/krisch/crm-backend/internal/legalEntities"
...
func DTOToBankAccDel(respObject oapi.DeleteLegalEntitiesRequestObject) le.BankAcc {
	var AccTodell le.BankAcc
	AccTodell.Bik = *respObject.Body.Name
	AccTodell.Address = *respObject.Body.Address
	AccTodell.CurAcc = *respObject.Body.CurAcc
	AccTodell.CorAcc = *respObject.Body.CorAcc
	AccTodell.Currency = *respObject.Body.Currency
	AccTodell.Comment = *respObject.Body.Comment
	AccTodell.Bik = *respObject.Body.Bik
	return AccTodell
}

func DTOToBankAccPatch(respObject oapi.PatchLegalEntitiesRequestObject) le.BankAcc {
	var AccTodell le.BankAcc
	AccTodell.Name = *respObject.Body.Name
	AccTodell.Address = *respObject.Body.Address
	AccTodell.CurAcc = *respObject.Body.CurAcc
	AccTodell.CorAcc = *respObject.Body.CorAcc
	AccTodell.Currency = *respObject.Body.Currency
...
func (a *Web) DeleteLegalEntities(ctx context.Context, respObject oapi.DeleteLegalEntitiesRequestObject) (oapi.DeleteLegalEntitiesResponseObject, error) {
	AccTodell := DTOToBankAccDel(respObject)
	ans, err := a.app.LegalEntitiesService.DeleteBankAccount(AccTodell)
	var bbb oapi.DeleteLegalEntities200JSONResponse
	bbb.ChangedCount = &ans.ChangedCount
	bbb.CorrAcc = &ans.CorrAcc
	return bbb, err
}

func (a *Web) GetLegalEntities(ctx context.Context, req oapi.GetLegalEntitiesRequestObject) (oapi.GetLegalEntitiesResponseObject, error) {
	accList, err := a.app.LegalEntitiesService.GetAllBankAccounts()
	lenSlice := len(accList)
	returnList := make(oapi.GetLegalEntities200JSONResponse, lenSlice, lenSlice)

	for i := range accList {
		var ba oapi.BankAccountDTO
		ba.Address = &accList[i].Address
		ba.Bik = &accList[i].Bik
		ba.Comment = &accList[i].Comment
		ba.CorAcc = &accList[i].CorAcc
		ba.CurAcc = &accList[i].CurAcc
		ba.Currency = &accList[i].Currency
		ba.Name = &accList[i].Name
...
func (a *Web) PatchLegalEntities(ctx context.Context, respObject oapi.PatchLegalEntitiesRequestObject) (oapi.PatchLegalEntitiesResponseObject, error) {
	AccToPatch := DTOToBankAccPatch(respObject)
	ans, err := a.app.LegalEntitiesService.UpdateBankAccount(AccToPatch)
	var bbb oapi.PatchLegalEntities200JSONResponse
	bbb.ChangedCount = &ans.ChangedCount
	bbb.CorrAcc = &ans.CorrAcc
	return bbb, err
}

func DTOToBankAccPost(respObject oapi.PostLegalEntitiesRequestObject) le.BankAcc {
	var AccTodell le.BankAcc
	if respObject.Body.Name != nil {
		AccTodell.Name = *respObject.Body.Name
	}
	if respObject.Body.Address != nil {
		AccTodell.Address = *respObject.Body.Address
	}
	if respObject.Body.CurAcc != nil {
		AccTodell.CurAcc = *respObject.Body.CurAcc
	}
	if respObject.Body.CorAcc != nil {
		AccTodell.CorAcc = *respObject.Body.CorAcc
	}
	if respObject.Body.Currency != nil {
		AccTodell.Currency = *respObject.Body.Currency
	}
	if respObject.Body.Comment != nil {
		AccTodell.Comment = *respObject.Body.Comment
	}
	if respObject.Body.Bik != nil {
		AccTodell.Bik = *respObject.Body.Bik
	}
...
func (a *Web) PostLegalEntities(ctx context.Context, respObject oapi.PostLegalEntitiesRequestObject) (oapi.PostLegalEntitiesResponseObject, error) {
	postAcc := DTOToBankAccPost(respObject)
	ans, err := a.app.LegalEntitiesService.CreateBankAccount(postAcc)
	var bbb oapi.PostLegalEntities201JSONResponse
	bbb.ChangedCount = &ans.ChangedCount
	bbb.CorrAcc = &ans.CorrAcc
	return bbb, err
}
